...
Vector3f Triangle::getSamplePoint(const RandomMwc& random) const
{
   // get two randoms
   ...
   // barycentrics
   return Vector3f( ((vertexs_m[1] - vertexs_m[0]) * a) + ((vertexs_m[2] - vertexs_m[0]) * b) + vertexs_m[0] );
}
...
float Triangle::getArea() const
{
   ...
   const Vector3f pa2( (vertexs_m[1] - vertexs_m[0]).cross(vertexs_m[2] - vertexs_m[1] ) );
