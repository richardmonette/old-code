...
{
   const float length = ::sqrtf( (xyz_m[0] * xyz_m[0]) +
                                 (xyz_m[1] * xyz_m[1]) +
                                 (xyz_m[2] * xyz_m[2]) );
...
{
   return Vector3f( xyz_m[0] >= 0.0f ? xyz_m[0] : -xyz_m[0],
...
Vector3f Vector3f::unitize() const
{
   ...
   return Vector3f( xyz_m[0] * oneOverLength,
                    xyz_m[1] * oneOverLength,
                    xyz_m[2] * oneOverLength );
}
...
{
   return bool( (xyz_m[0] == 0.0f) & (xyz_m[1] == 0.0f) & (xyz_m[2] == 0.0f) );
}
...
{
   return Vector3f( static_cast<float>(xyz_m[0] == v.xyz_m[0]),
