...
{
	std::cout << "Scene processing started...\n";
   ...
   groundReflection_m = skyEmission_m * groundReflection_m.getClamped( Vector3f::ZERO(), Vector3f::ONE() );
   ...
   std::cout << "Scene processing complete...\n";
}
...
{
	pIndex_m->getIntersection( rayOrigin, rayDirection, lastHit, pHitObject, hitPosition, hitUVT );
}

void Scene::getEmitter(const RandomMwc& random, Vector3f& position, const Triangle*& id) const
{
   if( !emitters_m.empty() )
   {
      // select emitter
      // not using lower bits, by treating the random as fixed-point i.f bits
      const dword index = ((static_cast<udword>(random.getDword()) & ((1u << MAX_EMITTERS_P) - 1)) * emitters_m.size()) >> MAX_EMITTERS_P;
