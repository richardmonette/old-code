...
{
   // default seeds
   const udword SEEDS[2] = { 521288629u, 362436069u };
}
...
{
   seeds_m[0] = (0 != seed) ? static_cast<udword>(seed) : SEEDS[0];
   seeds_m[1] = (0 != seed) ? static_cast<udword>(seed) : SEEDS[1];
}
...
/// queries --------------------------------------------------------------------
dword RandomMwc::getDword() const
{
   ...
   return static_cast<dword>( (seeds_m[0] << 16) + (seeds_m[1] & 0xFFFFu) );
}
