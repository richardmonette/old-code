...
void Flythrough::GetNextCameraPostion(Vec3 *p, Vec3 *l)
{
	...
	Vec3 deltaPoints;
	SubtractVec3(&deltaPoints, &points[stepmax], &points[stepmin]);
	Vec3ByScalar(&deltaPoints, (step - stepmin));
	Vec3DivideByScalar(&deltaPoints, (stepmax - stepmin));

	AddVec3(p, &points[stepmin], &deltaPoints);

	Vec3 deltaLook;
	SubtractVec3(&deltaLook, &looks[stepmax], &looks[stepmin]);
	Vec3ByScalar(&deltaLook, (step - stepmin));
	Vec3DivideByScalar(&deltaLook, (stepmax - stepmin));

	AddVec3(l, &looks[stepmin], &deltaLook);
