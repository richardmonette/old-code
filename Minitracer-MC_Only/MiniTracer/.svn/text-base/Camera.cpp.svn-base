   ...
   for( float y = height;  y-- > 0.0f; )
   {
      for( float x = width;  x-- > 0.0f; )
      {
         ...
         Vector3f sampleDirection;
         {
            ...
            const float xF = ((x + random.getFloat()) * 2.0f / width ) - 1.0f;
            const float yF = ((y + random.getFloat()) * 2.0f / height) - 1.0f;
            ...
            sampleDirection = (viewDirection_m +
               (offset * ::tanf(viewAngle_m * 0.5f))).unitize();
         }
         ...
         image.addToPixel( static_cast<dword>(x), static_cast<dword>(y),
            radiance );
      }
   }
}
