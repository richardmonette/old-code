...
) const
{
   ...
   const float tonemapScaling = calculateToneMapping( pixels_m, divider );
   ...
   for( udword i = 0;  i < pixels_m.size();  ++i )
   {
      ...
      {
         // tonemap
         float mapped = pixels_m[i][c] * divider * tonemapScaling;

         // gamma encode
         mapped = ::powf( (mapped > 0.0f ? mapped : 0.0f), GAMMA_ENCODE );

         // quantize
         mapped = ::floorf( (mapped * 255.0f) + 0.5f );

         // output as byte
         out << static_cast<ubyte>( mapped <= 255.0f ? mapped : 255.0f );
      }
   }
}
...
)
{
   ...
   float logMeanLuminance = 1e-4f;
   {
      ...
      {
         const float Y = pixels[i].dot( RGB_LUMINANCE ) * divider;
