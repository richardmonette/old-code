...
float4 PixelShaderFunction(VertexShaderOutput input) : COLOR0
{
	...
	float3 directionToLight = normalize(lightPosition - position);
    float2 cosAngles = cos(float2(spotOuterCone, spotInnerCone) * 0.5f);
	float spotDot = dot(-directionToLight, normalize(lightDirection));
	float spotEffect = smoothstep(cosAngles[0], cosAngles[1], spotDot);
	
	float intensity = lightIntensity * (1 - lightType) + (lightIntensity * spotEffect) * lightType;
