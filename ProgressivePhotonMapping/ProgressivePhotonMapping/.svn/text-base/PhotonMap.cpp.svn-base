...
*/
//**********************************************
int PhotonMap :: irradiance_estimate(
...
//***************************
{
  ...
  int s_theta = int( acos(s_dir[2])*(256.0/PI) );
  if (s_theta>255)
    node->s_theta = 255;
  else
   node->s_theta = (unsigned char)s_theta;

  int s_phi = int( atan2(s_dir[1],s_dir[0])*(256.0/(2.0*PI)) );
  if (s_phi>255)
    node->s_phi = 255;
  else if (s_phi<0)
    node->s_phi = (unsigned char)(s_phi+256);
  else
    node->s_phi = (unsigned char)s_phi;
}
