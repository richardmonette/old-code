...
*/
//************************************************
VolumePhotonMap :: VolumePhotonMap( const int max_phot )
//************************************************ 
{
  stored_photons = 0;
  prev_scale = 1;
  ...
  photons = (VolumePhoton*)malloc( sizeof( VolumePhoton ) * ( max_photons+1 ) );
...
*/
//**********************************************
int VolumePhotonMap :: irradiance_estimate(
...
//**********************************************
{
  ...
  np.index = (const VolumePhoton**)malloc( sizeof(VolumePhoton*)*(nphotons+1) );
  ...
  if (np.found <= 0)
  {
	  //std::cout << "VOLUMEPHOTONMAP: No photons found\n";
  ...
  float pdir[3];
	...
	{
		const VolumePhoton *p = np.index[i];
  ...
  const float tmp = (4/3) * PI * max_dist*max_dist*max_dist;    // estimate of density (volume)
...
*/
//******************************************
void VolumePhotonMap :: locate_photons(
  NearestVolumePhotons *const np,
  const int index ) const
//******************************************
{
  const VolumePhoton *p = &photons[index];
  float dist1;
  ...
  if ( dist2 < np->dist2[0] ) 
  {
	...
	{
		//std::cout << "found photon overflow\n";
		return;
    }
  }
}
...
//***************************
{
  ...
  {
	 // std::cout << "ERR: VOLUMEMAP ALREADY FULL\n";
    return;
  }

  stored_photons++;
  VolumePhoton *const node = &photons[stored_photons];
...
*/
//********************************************************
void VolumePhotonMap :: scale_photon_power( const float scale )
//********************************************************
{
 ...
 */
//******************************
void VolumePhotonMap :: balance(void)
//******************************
{
  if (stored_photons>1) {
    // allocate two temporary arrays for the balancing procedure
    VolumePhoton **pa1 = (VolumePhoton**)malloc(sizeof(VolumePhoton*)*(stored_photons+1));
    VolumePhoton **pa2 = (VolumePhoton**)malloc(sizeof(VolumePhoton*)*(stored_photons+1));
    ...
    int d, j=1, foo=1;
    VolumePhoton foo_photon = photons[j];
...
#define swap(ph,a,b) { VolumePhoton *ph2=ph[a]; ph[a]=ph[b]; ph[b]=ph2; }
...
//*****************************************************************
void VolumePhotonMap :: median_split(
  VolumePhoton **p,
...
//****************************
void VolumePhotonMap :: balance_segment(
  VolumePhoton **pbal,
  VolumePhoton **porg,
  const int index,
  const int start,
